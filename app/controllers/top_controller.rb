class TopController < ApplicationController
  def index
    ...
    if feed_type == nil
      @new_feed_items = Post.where("created_at >= ?", now.ago(31.days)).limit(6).order("created_at desc")
      ...
      @feed_items = Post.where("created_at >= ?", now.ago(31.days)).order("created_at desc").page(params[:page]).per(12)
    end
  end
end
